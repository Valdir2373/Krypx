 ...
 * Suporta drag, resize por bordas, minimize, maximize e close com kill de processo.
 */
...
/* Recalcula content_* a partir de x/y/w/h */
...
/* Detecta borda de resize: retorna direção ou RESIZE_NONE */
static uint8_t hit_resize_dir(window_t *win, int x, int y) {
    ...
    /* Só resize lateral abaixo da titlebar */
...
static void wm_draw_titlebar(window_t *win) {
    ...
    /* Botão maximizar/restaurar (verde) */
    bx -= BTN_SIZE + 4;
...
static void draw_cursor(int x, int y) {
    /* Seta preenchida apontando para o canto superior esquerdo */
    int i;
    /* Sombra preta */
    for (i = 0; i < 13; i++) {
        int len = 13 - i;
...
void wm_close(window_t *win) {
    ...
    /* Mata o processo associado e libera sua memória */
    if (win->proc_pid != 0) {
...
void wm_mouse_down(int x, int y, uint8_t btn) {
    ...
    /* Verificar borda de resize primeiro */
    ...
    if (!(win->flags & WIN_NO_TITLEBAR) &&
        ...
        /* Botão fechar (vermelho — mais à direita) */
        int bx = win->x + win->w - BTN_SIZE - BTN_MARGIN;
        ...
        if (x >= bx && x < bx + BTN_SIZE && y >= by_btn && y < by_btn + BTN_SIZE) {
            ...
            /* Foco passa para a janela abaixo */
            if (wm_focused == win) {
        ...
        if (x >= bx && x < bx + BTN_SIZE && y >= by_btn && y < by_btn + BTN_SIZE) {
            if (win->flags & WIN_MAXIMIZED) {
                ...
                win->y = 0;
                win->w = (int)fb.width;
                win->h = (int)fb.height - 40;  /* 40 = TASKBAR_HEIGHT */
                win->flags |= WIN_MAXIMIZED;
            }
            update_content(win);
            return;
        ...
        /* Clicou na titlebar mas fora dos botões → inicia drag */
...
void wm_draw_taskbar_entries(int x, int y, int bw, int bh, int max_x) {
    ...
    while (w) {
        if (w->used && (w->flags & WIN_MINIMIZED)) {
            ...
            /* Título (truncado ao espaço disponível) */
