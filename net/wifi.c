...
/* WiFi state machine — manages scan, connect, WPA2 handshake */
...
/* Internal WPA2 context */
static uint8_t g_pmk[32];
static uint8_t g_ptk[48];   /* KCK[0..15] KEK[16..31] TK[32..47] */
...
/* Forward declaration of driver send — implemented in rtl8188eu.c */
extern void rtl8188eu_send_frame(const uint8_t *frame, uint16_t len);

/* ── 802.11 frame building ─────────────────────────────────────────────────── */

/* Minimal 802.11 Authentication frame (open system, seq 1) */
static uint16_t build_auth_req(const uint8_t *bssid, const uint8_t *my_mac,
                                uint8_t *out) {
    memset(out, 0, 30);
    out[0] = 0xB0; out[1] = 0x00;  /* frame control: auth */
    out[2] = 0x00; out[3] = 0x00;  /* duration */
    memcpy(out+4,  bssid,  6);      /* DA */
    memcpy(out+10, my_mac, 6);      /* SA */
    memcpy(out+16, bssid,  6);      /* BSSID */
    out[22]=0x00; out[23]=0x00;     /* seq ctrl */
    /* Body: alg=open(0), seq=1, status=0 */
    out[24]=0; out[25]=0; out[26]=1; out[27]=0; out[28]=0; out[29]=0;
    return 30;
}

/* Minimal 802.11 Association Request frame */
static uint16_t build_assoc_req(const uint8_t *bssid, const uint8_t *my_mac,
                                  const char *ssid, uint8_t ssid_len,
                                  uint8_t *out) {
    uint16_t pos = 0;
    memset(out, 0, 256);
    out[pos++]=0x00; out[pos++]=0x00; /* frame ctrl: assoc req */
    out[pos++]=0x00; out[pos++]=0x00; /* duration */
    memcpy(out+pos, bssid,  6); pos+=6; /* DA */
    memcpy(out+pos, my_mac, 6); pos+=6; /* SA */
    memcpy(out+pos, bssid,  6); pos+=6; /* BSSID */
    out[pos++]=0x00; out[pos++]=0x00;   /* seq ctrl */
    /* Body: capability(0x0431), listen interval(10) */
    ...
    out[pos++]=0x01; out[pos++]=0x00; /* version */
    /* group cipher: CCMP */
    out[pos++]=0x00; out[pos++]=0x0F; out[pos++]=0xAC; out[pos++]=0x04;
    /* pairwise count=1, CCMP */
...
/* Build EAPOL frame wrapped in 802.11 data frame */
static uint16_t build_eapol_data_frame(const uint8_t *bssid, const uint8_t *my_mac,
                                        const uint8_t *eapol, uint16_t elen,
                                        uint8_t *out) {
    uint16_t pos = 0;
    out[pos++]=0x08; out[pos++]=0x01; /* data, to-DS */
    out[pos++]=0x00; out[pos++]=0x00;
    memcpy(out+pos, bssid, 6);  pos+=6; /* DA=BSSID */
    memcpy(out+pos, my_mac, 6); pos+=6; /* SA */
    memcpy(out+pos, bssid, 6);  pos+=6; /* BSSID */
    out[pos++]=0x00; out[pos++]=0x00;
    /* LLC/SNAP + EAPOL */
...
/* Called by RTL driver when a beacon/probe-response is received */
...
/* Called by driver when an EAPOL frame arrives (WPA2 4-way handshake) */
void wifi_on_eapol(const uint8_t *data, uint16_t len) {
    ...
    if ((info & EAPOL_KEY_INFO_ACK) && !(info & EAPOL_KEY_INFO_MIC)) {
        /* Message 1: AP → STA with ANonce */
        memcpy(g_anonce, k->nonce, 32);

        /* Replay counter from msg1 */
        g_replay_ctr = 0;
        ...
        /* Message 3: verify MIC, send message 4 */
        /* We trust the AP for now — send msg4 */
        int i; for(i=0;i<8;i++) g_replay_ctr=(g_replay_ctr<<8)|((uint8_t*)&k->replay_ctr)[i];
...
/* Dispatch incoming 802.11 frame */
    ...
    if (type == 0) { /* management */
        if (subtype == 8 || subtype == 5) { /* beacon or probe response */
            if (g_wifi_state == WIFI_STATE_SCANNING) {
                /* Parse beacon: addr3=BSSID, body starts at byte 36 */
                ...
                while (i+1 < blen) {
                    ...
                    if (id == 0 && elen <= 32) { /* SSID */
                        memcpy(ssid, body+i+2, elen);
                        ssid_len = elen;
                        ssid[elen] = '\0';
                    } else if (id == 3 && elen == 1) { /* DS channel */
                        channel = body[i+2];
                        ...
                        sec = WIFI_SEC_WPA; /* WPA vendor IE */
                    }
                    i += 2 + elen;
                }
                int8_t rssi = -70; /* placeholder — driver should fill in */
                if (ssid_len > 0)
                    wifi_on_beacon(bssid, ssid, ssid_len, rssi, channel, sec);
            }
        } else if (subtype == 0x0B) { /* authentication response */
            uint16_t seq    = (uint16_t)((frame[26])|(frame[27]<<8));
            uint16_t status = (uint16_t)((frame[28])|(frame[29]<<8));
            if (seq == 2 && status == 0) {
                /* Auth OK → send association request */
                wifi_network_t *net = &g_wifi_networks[g_connect_idx];
                extern uint8_t net_mac[6];
                uint8_t out[256];
        ...
        uint16_t hdr_len = 24;
        if (fc1 & 0x08) hdr_len = 30; /* QoS data */
